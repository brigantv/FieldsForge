...
#SBATCH -p htc         # Use the "compute" partition
#SBATCH -J MD     # Job name

#modules to upload
module load openmpi/4.1.6-gcc-8.5.0-j2yerfm
module load netlib-scalapack/2.2.0-gcc-8.5.0-2say5eg
module load netlib-lapack/3.8.0-gcc-8.5.0-beua4ey

#########################################	 USER SECTION (MODIFY ONLY THIS PART)

export WORK_DIR=/home/users/brigantv/active_learning		### directory where you put the subdir related to the specific compound under study
export SNAP_EXE_DIR=/home/users/brigantv/active_learning	### directory where the executable of snap is located (exe_name: snap)
...
ABINIT_EXE_DIR=/home/users/brigantv/cp2k-2024.3/exe/local	### directory containing the ab initio (CP2K) executable
project_name_cp2k=benzene					### flag specific to CP2K
from_scratch="yes"						### if set to "yes", deletes the common Record.txt file, restarting the AL session from scratch
consecutive_runs=10						### number of consecutive successful runs that will lead to the conclusion of the AL session
...
FILE_ENE_TO_COPY=ener_tr_${nconfig}				###these files contain your starting data
FILE_DIP_TO_COPY=dipoles_tr_${nconfig}
FILE_GEO_TO_COPY=geo_tr_${nconfig}
FILE_FORCE_TO_COPY=forces_tr_${nconfig}
...
node_name="${nodes[$k]}"
   ...
   TMP_DIR_NODE="/tmp/AL_${i}"  # Unique TMP_DIR per node/tas
   ...
   rm -f ${TMP_DIR_NODE}/*
    ...
    cp ${WORK_DIR}/${compound}/*wfn ${TMP_DIR_NODE}
...
# Loop to modify files and run tasks in the dynamically created directories
for i in $(seq ${init_AL} ${num_AL} ); do

    (( k=${i}-1 ))
    node_name="${nodes[$k]}"
    TMP_DIR_NODE="/tmp/AL_${i}"
    srun -N1 -n1 --nodelist=$node_name bash -c "
    cd ${TMP_DIR_NODE}
